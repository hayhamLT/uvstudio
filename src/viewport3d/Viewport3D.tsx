...
function CameraRig() {
  ...
  useEffect(() => {
    ...
    const cx = (mnx + mxx) / 2,
      cy = (mny + mxy) / 2,
      cz = (mnz + mxz) / 2
    const r = Math.max((mxx - mnx) / 2, (mxy - mny) / 2, (mxz - mnz) / 2, 0.5)
    const fovHalf = (camera.fov * Math.PI) / 180 / 2
    const dist = (r / Math.tan(fovHalf)) * 1.35
    const dir = new THREE.Vector3(1, 0.65, 1).normalize()
    const pos: [number, number, number] = [cx + dir.x * dist, cy + dir.y * dist, cz + dir.z * dist]
...
function View3dToolbar() {
  ...
  const modes: { id: 'shaded' | 'distortion' | 'checker'; label: string; hot: string; icon: ReactElement }[] =
    [
      {
        id: 'shaded',
        label: 'Shaded (content)',
        hot: '1',
        icon: <circle cx="12" cy="12" r="7.5" fill="currentColor" stroke="none" />,
      },
      {
        id: 'distortion',
