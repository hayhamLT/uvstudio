    ...
    <div className="animate-float-up absolute left-3 top-[54px] z-40 w-64">
      ...
      <div className="glass rounded-lg border border-brand-500/30 p-3 shadow-xl">
        <div className="text-[12.5px] leading-snug text-fog-100">
          <span className="font-semibold text-brand-300">Start here</span> — Auto-map fits every screen to its
          linked content in one click.
        </div>
        <button
          onClick={dismiss}
    ...
    <header className="relative z-50 flex h-12 items-center gap-2 border-b border-line bg-ink-900/80 px-3 backdrop-blur">
      {hasModel && (
        <>
          ...
          <button
            onClick={() => {
              if (mappedCount) fromC4D ? sendToC4D() : exportGltf()
            }}
