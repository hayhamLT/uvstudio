...
export default function Landing() {
  ...
  return (
    <div ref={dropRef} className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden">
      ...
      >
        <span className="relative flex h-20 w-20 items-center justify-center transition-transform duration-500 ease-out group-hover/orb:scale-105 group-active/orb:scale-95">
          ...
          <span className={`absolute inset-1.5 rounded-full border border-brand-500/25 animate-orb-drift ${dragging ? 'border-brand-400/60' : ''}`} />
          {/* breathing core */}
          <span className="animate-orb-breathe">
            <span
              className={`relative flex h-12 w-12 items-center justify-center rounded-full bg-linear-to-br from-brand-400 to-brand-600 text-ink-950 shadow-lg shadow-black/30 transition-transform duration-300 ${
                dragging ? 'scale-110' : 'group-hover/orb:scale-110'
              }`}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-300 group-hover/orb:-translate-y-0.5">
                {dragging ? (
